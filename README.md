...
[![Gitter](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/cogitatio/vagrant-mutagen?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge)
[![Twitter](https://img.shields.io/twitter/url/https/github.com/cogitatio/vagrant-mutagen.svg?style=social)](https://twitter.com/intent/tweet?text=Checkout%20this%20awesome%20Vagrant%20plugin!&url=https%3A%2F%2Fgithub.com%2Fcogiatio%2Fvagrant-mutagen&hashtags=mutagen,vagrant)
...
By setting the `config.mutagen.remove_on_suspend  = false`, **suspend** and **halt** will not remove them. 
...
To skip adding some entries to the /etc/hosts file add `mutagen: "skip"` option to network configuration:

    config.vm.network "private_network", ip: "172.21.9.9", mutagen: "skip"
...
To keep your /etc/hosts file unchanged simply add the line below to your `VagrantFile`:
    ...
    config.mutagen.remove_on_suspend = false
    
This disables vagrant-mutagen from running on **suspend** and **halt**.
...
To allow vagrant to automatically update the hosts file without asking for a sudo password, add one of the following snippets to a new sudoers file include, i.e. `sudo visudo -f /etc/sudoers.d/vagrant_mutagen`.

For Ubuntu and most Linux environments:

    # Allow passwordless startup of Vagrant with vagrant-mutagen.
...
```
git clone https://github.com/cogitatio/vagrant-mutagen
cd vagrant-mutagen
